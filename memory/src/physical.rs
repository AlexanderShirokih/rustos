...
impl FrameBitmap {
    ...
    fn new(placement_address: PhysicalAddress, target_region: &MemoryRange) -> Self {
    ...
    pub(crate) fn size_for(region: &MemoryRange) -> usize {
...
impl PhysicalMemoryManager {
    ...
    pub fn new(memory: &MemoryRange) -> Self {
        ...
        let ppm_self_region =
            MemoryRange::new(ppm_self_region_begin, ppm_self_region_end, frame_size);
    ...
    pub fn mark_used_region(&self, region: &MemoryRange) {
...
impl FrameAllocator for PhysicalMemoryManager {
    ...
    fn allocate_frames_exact(
        ...
        let mut bitmap = self.allocated_frames.lock();
        let frame = Frame::containing_address(physical_address, self.frame_size());

        for idx in frame.number()..frame.number() + size {
            bitmap.set(idx);
        }
