...
impl FrameBitmap {
    ...
    /// Релоцирует внутренний указатель bitmap в higher-half.
    ///
    /// # Safety
    ///
    /// Вызывать ровно один раз после включения MMU, когда память уже
    /// отображена в higher-half с линейным смещением `offset`.
    pub unsafe fn relocate_ptr_by_offset(&mut self, offset: usize) {
        ...
        // SAFETY: raw_new указывает на тот же физический буфер, но в virtual space.
        // Длина сохраняется прежней.
        self.bitmap = unsafe { Box::from_raw(raw_new) };
    }
}
