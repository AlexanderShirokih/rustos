...
| Крейт                | Роль                                                          |
|----------------------|---------------------------------------------------------------|
| `lib/runtime`        | тонкие svc-обёртки syscall-ABI; null-аллокатор; транспорт IPC |
...
Глобальный аллокатор процесса — `NoHeap`: его `alloc` всегда возвращает null,
динамическая аллокация завершается отказом. Данные живут на стеке и в статике,
коллекции — фиксированной ёмкости.
