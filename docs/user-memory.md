...
- `MemoryMapper::map` падает (например, OOM фреймов) ->
  `release_pending` снимает регион с учёта. bump-указатель **не**
  откатывается: `MemoryMapper::map` не транзакционен и при partial
  failure часть страниц региона уже могла оказаться в page-tables;
  реюз того же VA дал бы `AlreadyMapped` и оставил бы фреймы прибитыми
  до полного exit процесса. VA сгорает — фреймы из частичного маппинга
  остаются прибитыми к процессу до Drop AS.
...
Стратегия — bump-pointer: освобождение текущим интерфейсом
`MemoryMapper` не поддерживается (`unmap` возвращает `Unsupported`),
поэтому реюзить слоты не имеет смысла. Когда unmap будет реализован,
стратегию можно заменить на interval-set без изменения публичного API.
...
1. **Нет `vm_free`/`vm_unmap`.** `MemoryMapper::unmap` возвращает
   `Unsupported`, поэтому syscall не вводится. Освобождение —
   только полный exit процесса (Drop AS возвращает все фреймы
   аллокатору в page-table walk-е).
2. **Bump-стратегия.** Освобождённые слоты не реюзятся; в long-running
   процессе с многими `vm_allocate` диапазон рано или поздно
   исчерпается — процесс получит `OutOfMemory`. Эволюция стратегии
   запланирована вместе с `unmap`.
3. **Защита целиком на регион.** `vm_remap` требует точного совпадения
   `(base, size)` с выделенным регионом.
4. **AArch64 `remap`.** Платформенная реализация инвалидирует TLB через
   `tlbi va, asid`. Для kernel-AS используется `va_global` —
   user-syscall'ы туда не достают (kernel-AS не имеет
   `UserVmAllocator`).
