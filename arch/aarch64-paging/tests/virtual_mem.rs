...
use common::{
    TEST_FRAME_SIZE, basic_layout, build_frame_allocator, layout_from_regions, make_region,
    mock_backend,
};
...
fn map_and_translate_single_page() {
    ...
    let layout = basic_layout(32, total_frames - 32);
    let manager =
        PageTableManager::new(&allocator, &backend, layout.heap).expect("manager should init");
...
fn map_range_maps_multiple_pages() {
    ...
    let backend = mock_backend(total_frames);
    let allocator = build_frame_allocator(&backend, total_frames, &[]);
    let layout = basic_layout(64, total_frames - 64);
    let manager =
        PageTableManager::new(&allocator, &backend, layout.heap).expect("manager should init");
...
fn identity_regions_are_direct_mapped() {
    ...
    let backend = mock_backend(total_frames);
    let allocator = build_frame_allocator(&backend, total_frames, &[]);
    let kernel_region = make_region("kernel", 4, 4, EntryFlags::KERNEL_CODE);
    let dtb_region = make_region("dtb", 8, 2, EntryFlags::DEVICE);
    ...
    let manager =
        create_page_table_manager(&allocator, &backend, layout, &identity).expect("manager init");
...
fn adjacent_identity_regions_with_misaligned_boundaries_fail() {
    ...
    let result = create_page_table_manager(&allocator, &backend, layout, &identity);
    match result {
...
fn adjacent_identity_regions_with_aligned_boundaries_succeed() {
    ...
    let manager = create_page_table_manager(&allocator, &backend, layout, &identity)
...
fn verify_leaf_entry_after_map(
    ...
    manager: &PageTableManager<
        '_,
        memory::physical_manager::PhysicalMemoryManager<'_, MockMemoryBackend>,
        MockMemoryBackend,
    >,
    page: Page,
    frame: Frame,
