...
fn dispatch(op: SyscallOp, args: [u64; 6]) -> Result<u64, SyscallError> {
    ...
    if frame.ret >= 0 {
        ...
        // Decode `-(SyscallError as u32 as i64)` обратно.
...
fn thread_create_does_not_create_thread_on_out_of_handles() {
    ...
    // Таблица c capacity=2: один слот под process-handle, второй заполнен dummy.
...
fn process_start_succeeds_when_drain_frees_caller_slot() {
    ...
    // Таблица capacity=2: process-handle + bootstrap-handle (с TRANSFER).
    // Свободных слотов нет; drain bootstrap-handle-а освободит ровно один.
    let table = Arc::new(MutexCell::new(HandleTable::with_capacity(2)));
    let process_id = install_process_handle(&table);
    ...
    // Готовим user-память: 4 байта = HandleId bootstrap-handle-а.
    ...
    assert!(ret != 0, "returned thread-handle must be non-zero");
    assert_eq!(rt.start_user_process_calls.load(Ordering::SeqCst), 1);
    // Bootstrap-handle ушёл из caller-table; thread-handle занял слот.
    table.with_lock(|tbl| {
