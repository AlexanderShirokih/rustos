...
fn channel_full_api() {
    ...
    qemu_test_harness::kassert!(observed & CHANNEL_READABLE != 0);

    let msg = channel_read(right).expect("right reads ping");
    qemu_test_harness::kassert_eq!(msg.bytes(), b"ping");
    ...
    qemu_test_harness::kassert_eq!(msg.bytes(), b"pong");
    ...
    qemu_test_harness::kassert!(observed & CHANNEL_PEER_CLOSED != 0);
    qemu_test_harness::kassert!(matches!(channel_read(right), Err(IpcError::PeerClosed)));
