...
//! [`KernelRuntime::current_user_vm`], валидирует аргументы, делегирует
...
/// аргумент syscall'а. Неизвестное значение -> [`SyscallError::InvalidArgument`].
...
impl UserMemFlags {
    pub const fn from_raw(raw: u64) -> Result<Self, SyscallError> {
        match raw {
            0 => Ok(Self::ReadWrite),
            1 => Ok(Self::ReadOnly),
...
pub fn sys_memory_allocate(size_bytes: u64, flags_raw: u64) -> Result<u64, SyscallError> {
    ...
    let flags = UserMemFlags::from_raw(flags_raw)?.to_mem_flags();
...
mod tests {
    ...
    fn user_mem_flags_from_raw_unknown_is_invalid() {
        assert_eq!(
            UserMemFlags::from_raw(3),
            Err(SyscallError::InvalidArgument)
        );
        assert_eq!(
            UserMemFlags::from_raw(u64::MAX),
            Err(SyscallError::InvalidArgument)
        );
    }
