//! bootstrap-log: дренаж кадров (hello, heartbeat, unknown) и выход по
...
use userland_abi::{
    BOOTSTRAP_ABI_VERSION, BOOTSTRAP_HELLO_MAGIC, BOOTSTRAP_HELLO_SIZE, encode_bootstrap_heartbeat,
};
...
fn bootstrap_log_exits_on_peer_close() {
    ...
    let heartbeat = encode_bootstrap_heartbeat(7);
    peer.write(Message::from_bytes(&heartbeat).expect("heartbeat fits inline"))
        .expect("heartbeat write must succeed");
