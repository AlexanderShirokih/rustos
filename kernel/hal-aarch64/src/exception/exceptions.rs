...
// Asm в `exception_entry!` обращается к sp_el0 по литеральному offset-у.
const _: () = assert!(core::mem::offset_of!(ExceptionFrame, sp_el0) == 288);
...
pub struct ExceptionFrame {
    ...
    pub far: u64,
    /// User-указатель стека (SP_EL0). Живёт в железе между трапами, поэтому
    /// сохраняется на входе и восстанавливается перед `eret`.
    pub sp_el0: u64,
    /// Выравнивание фрейма до кратности 16.
    _reserved: u64,
}
...
fn write_frame(buf: &mut StaticString<EXCEPTION_BUF_SIZE>, frame: &ExceptionFrame) {
    ...
    let _ = writeln!(buf, "  SP_EL0: {:#018x}", frame.sp_el0);
    ...
    ($name:ident, $kind:expr) => {
        ...
        extern "C" fn $name() {
            naked_asm!(
                ...
                // Системные регистры: ELR, SPSR, ESR, FAR, SP_EL0
                "mrs x9,  elr_el1",
                "mrs x10, spsr_el1",
                "stp x9,  x10, [sp, #256]",
                "mrs x9,  esr_el1",
