...
fn channel_full_api() {
    ...
    kernel_tests::kassert_eq!(msg.bytes(), b"ping");
    ...
    kernel_tests::kassert_eq!(msg.bytes(), b"pong");
    ...
    kernel_tests::kassert!(observed & CHANNEL_PEER_CLOSED != 0);
    kernel_tests::kassert!(matches!(channel_read(right), Err(IpcError::PeerClosed)));
