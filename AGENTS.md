...
- **Никакого `unsafe` в архитектурно-независимом коде.** Запрет автоматизирован: `[workspace.lints.rust] unsafe_code = "deny"`. Платформенные крейты (`hal-*`, `drivers-*-aarch64`, `qemu-test-harness-aarch64`) опциируют разрешение целиком через `#![allow(unsafe_code)]` в корне крейта. В архитектурно-независимых крейтах (`memory`, `kernelspace`, `scheduler`, `kobject`, `syscall`, `io`, `collections`, `fdt`, `drivers-common`, `qemu-test-harness`) `#![allow(unsafe_code)]` ставится **точечно — на уровне отдельного файла**, только если без `unsafe` обойтись невозможно (kernel-mechanism: allocators, scheduler, MMIO, raw-pointer relocation). Новый файл и новый крейт по умолчанию запрещают `unsafe` — добавление `#![allow]` видно в diff и требует обоснования при ревью.
